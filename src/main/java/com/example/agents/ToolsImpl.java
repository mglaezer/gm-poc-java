...
public class ToolsImpl implements Tools {
    ...
    public CustomerProfile analyzeCustomerNeeds(int familySize, String primaryUsage, List<String> preferences) {
        ...
        boolean needsOffRoad = preferences.stream().anyMatch(p -> p.toLowerCase().contains("offroad"));
        ...
        String fuelPreference = preferences.stream()
            .filter(p -> p.toLowerCase().contains("electric") || p.toLowerCase().contains("hybrid"))
            .findFirst()
            .orElse("gasoline");
    ...
    public CustomerProfile buildCustomerProfile(CustomerRequirements requirements) {
        List<VehicleCategory> categories = suggestVehicleCategories(
            new CustomerProfile(
        ...
        return new CustomerProfile(
            ...
            requirements.budget(),
            categories,
            requirements.mustHaveFeatures().stream().anyMatch(f -> f.toLowerCase().contains("towing")),
            requirements.mustHaveFeatures().stream().anyMatch(f -> f.toLowerCase().contains("offroad")),
            "gasoline"
        );
    }
    ...
    public VehicleComparison compareToCompetitors(String vehicleId) {
        ...
        points.add(new ComparisonPoint("MPG Combined", vehicleId, String.valueOf((vehicle.mpgCity() + vehicle.mpgHighway()) / 2)));
        points.add(new ComparisonPoint("MPG Combined", "COMP001", String.valueOf(((vehicle.mpgCity() + vehicle.mpgHighway()) / 2) - 2)));
        
        return new VehicleComparison(
            competitors,
            List.of("Price", "MPG Combined", "Warranty", "Features"),
            points
        );
    }
    ...
    public NegotiationStrategy suggestNegotiationStrategy(String vehicleId, MarketConditions marketConditions) {
        ...
        if ("high".equals(marketConditions.inventoryLevel())) leveragePoints.add("High inventory - dealer motivated to move units");
